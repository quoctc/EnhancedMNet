...
from utils.common import conv1x1_block, conv3x3_block, conv3x3_dw_block, conv5x5_dw_block, SEUnit, Classifier
from models.utils.Filter import Filter
...
class MobileNetV1(torch.nn.Module):
    ...
    def __init__(self,
        ...
        print(f'Filter is using: {use_filter}')
        print(f'Small input: {init_conv_stride == 1}')
        print(f'Number of Gabor filters: {num_gabor_filters}')

        # Initialize filter_instance
        self.use_filter = use_filter
        self.filter_instance = Filter(device=self.device, num_gabor_filters=num_gabor_filters)
        
        in_channels = 0
        if 'Gabor' in self.use_filter:
            in_channels += num_gabor_filters
        if 'Sobel' in self.use_filter:
            in_channels += 2
        if 'LoG' in self.use_filter:
            in_channels += 2
        if 'Img' in self.use_filter:
            in_channels += 3
        if 'NA' in self.use_filter:
            in_channels = 3
    ...
    def forward(self, x):
        ...
        filter_outs = None
        if 'Gabor' in self.use_filter:
            filter_outs = self.filter_instance.get_gabor_filtered_outputs(x, use_grayscale=True)
        if 'Sobel' in self.use_filter:
            filter_outs = self.filter_instance.get_sobel_filtered_outputs(x)
        if 'LoG' in self.use_filter:
            filter_outs = self.filter_instance.get_log_filtered_outputs(x)

        if 'Img' in self.use_filter and filter_outs is not None:
            combined_out = torch.cat([x] + filter_outs, dim=1)
        elif 'NA' in self.use_filter:
            combined_out = x
        else:
            combined_out = torch.cat([] + filter_outs, dim=1)
...
def build_mobilenet_v1(num_classes, width_multiplier=1.0, cifar=False, use_filter='Gabor',
                 device_id=0, num_gabor_filters = 8):
    """
...
def build_mobilenet_v2(num_classes, width_multiplier=1.0, cifar=False, use_filter='Gabor',
                 device_id=0, num_gabor_filters = 8):
    """
...
def build_mobilenet_v3(num_classes, version, width_multiplier=1.0, cifar=False, use_lightweight_head=True, use_filter='Gabor',
                 device_id=0, num_gabor_filters = 8):
    """
