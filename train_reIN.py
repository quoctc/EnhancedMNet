...
path_ReIN = '../../datasets/ImageNetForRescale_multi_acc'
...
parser.add_argument('-d', '--dataset', type=str, choices=['imagenet_reIN'], default='imagenet_reIN', help='Dataset name.')
...
parser.add_argument('--seed', default=None, type=int, nargs='+', help='seed for initializing training. ')
parser.add_argument('--gpu', default=None, type=int, help='GPU id to use.')
parser.add_argument('--ksize', default=None, type=list, help='Manually select the eca module kernel size')
...
def main():
    ...
    for rescaled_set in arr_rescaled_set:
        ...
        for typesize in arr_typesize_groups:
            args.arch = 'ReIN' + str(rescaled_set) + '_ImageNet100_MobileNet_w_Filters__' + args.architecture  + args.filter +'_128_multiGPU'
            filenameLOG = "./checkpoints/%s/"%(args.arch + '_' + args.action) + '/' + args.arch + '.txt'
            os.makedirs(os.path.dirname(filenameLOG), exist_ok=True)
            
            input_size, _ = get_input_size_and_classes(args)
            num_class = rescaled_set
            model = get_model(args, input_size, num_class)
...
def get_model(args, input_size, num_class):
    """"
    Return the model for the given architecture.
    """
